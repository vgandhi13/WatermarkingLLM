...
def generate_response_binarize(model, tokenizer, prompt, length=30):
    ...
    for i in range(length):
        ...
        token = torch.tensor([[token_id]], device=inputs.device)
...
def generate_payloaded_response(key, model, tokenizer, prompt, payload, length=30, threshold=2, bit_limit=None, temperature=1.0):
    ...
    for i in range(length):
        ...
        token = torch.tensor([[inv_perm[token_id]]], device=inputs.device)
        inputs = torch.cat([inputs, token], dim=-1)
...
if __name__ == '__main__':
    ...
    model, tokenizer = start_model("mistralai/Mistral-7B-v0.1")
    ...
    response_sizes = [100, 200, 300]
    samples_per_size = 5 # Set to 10 for a quicker run
    #compute precision_send
    avg = 0
    for size in response_sizes:
        ...
        for i in range(samples_per_size):
            ...
            for prompt in prompts:
                res, ecc = generate_payloaded_response(random.random(), model, tokenizer, prompt,
            ...
            print("Average bits sent for " + str(size) + " tokens:" + str(acc/len(prompts)))
            avg+=(acc/len(prompts))
