...
def start_model(model_name="gpt2"):
    print("Starting model on device: ", torch.cuda.get_device_name(0))
    ...
    # Optimize model loading with device_map and torch_dtype
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        device_map="cuda",  # Automatically distribute model across available GPUs
        trust_remote_code=True  # Allow loading of custom models
    ).to(device)
    loading_time = time.time() - start_time
    print(f"Model loaded (took {loading_time:.2f} seconds)", flush=True)
