        ...
        for i in range(prompt_length - 1, prompt_length - 1 + max(0, generated_length - 1)):
            ...
            if question_mark != '?':
                ...
                if self.hash_scheme == 'hashlib':
                    ...
                    # idx = new_index = self.loaded_wrapper.predict(avg_embedding, "/work/pi_adamoneill_umass_edu/WatermarkingLLM/kmeans_model_2040_n3_minibatch.pkl") % len(codeword)
            ...
            if prob_of_next_token < t:
                ...
                question_mark = '?'
                bit = '?'
                denom = prob_of_next_token - prob_of_start_of_token
                if denom == 0:
                    t = 0.5
                else:
                    t = (t - prob_of_start_of_token) / denom
