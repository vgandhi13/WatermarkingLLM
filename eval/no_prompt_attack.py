...
def run_experiment(crypto_scheme, enc_method):
    ...
    csv_filename = f'no_prompt_attack_outputs_{crypto_scheme}_{enc_method}_{timestamp}.csv'
...
def main():
    ...
    for (crypto_scheme, enc_method), color in zip(configs, colors):
        print(f"\nRunning experiment with {crypto_scheme} and {enc_method} encoding...")
        precision, recall, metrics = run_experiment(crypto_scheme, enc_method)
        ...
        if len(precision) > 1 and len(recall) > 1:
            ...
            print(f"Skipping plot for {crypto_scheme} - {enc_method} due to lack of data.")
    ...
    plt.savefig('precision_recall_curves_comparison_no_prompt.png', 
